...
namespace Transmit.Utils
{
    internal static class PacketIo
    {
        ...
        public static int ReadPacketHeader(byte[] packetBuffer, int offset, int bufferLength, out byte channelID,
            out ushort sequence, out ushort ack, out uint ackBits)
        {
            if (bufferLength < 4)
            ...
            using (var reader = ByteArrayReaderWriter.Get(packetBuffer))
            {
                ...
                var prefixByte = reader.ReadByte();
                if ((prefixByte & 1) != 0)
                ...
                if ((prefixByte & (1 << 5)) != 0)
                {
                    if (bufferLength < 2 + 1) throw new FormatException("Buffer too small for packet header");

                    var sequenceDiff = reader.ReadByte();
                    ack = (ushort) (sequence - sequenceDiff);
                }
                else
                {
                    if (bufferLength < 2 + 2) throw new FormatException("Buffer too small for packet header");
        ...
        public static int ReadFragmentHeader(byte[] packetBuffer, int offset, int bufferLength, int maxFragments,
            int fragmentSize, out int fragmentID, out int numFragments, out int fragmentBytes,
            out ushort sequence, out ushort ack, out uint ackBits, out byte channelID)
        {
            ...
            using (var reader = ByteArrayReaderWriter.Get(packetBuffer))
            {
                ...
                if (fragmentID == 0)
                {
                    var packetHeaderBytes = ReadPacketHeader(packetBuffer, Defines.FRAGMENT_HEADER_BYTES, bufferLength,
                        out packetChannelID, out packetSequence, out packetAck, out packetAckBits);
        ...
        public static int WritePacketHeader(byte[] packetBuffer, byte channelID, ushort sequence, ushort ack,
            uint ackBits)
        {
            using (var writer = ByteArrayReaderWriter.Get(packetBuffer))
            {
